...
// Gao Wang (c) 2018 gaow@uchicago.edu
#include "RcppGSL.h"
#include <iostream>
extern "C" {
#include <stdio.h>
...
bool * int2bool(RcppGSL::vector<int> & a, int K)
{
...
// [[Rcpp::export]]
Rcpp::List extreme_deconvolution_rcpp(
                                      ...
                                      int noprojection, int diagerrors, int noweights)
{
	...
	for (ii = 0; ii != N; ++ii) {
		...
		if (!noproj)
			for (dd1 = 0; dd1 != dy; ++dd1)
				for (dd2 = 0; dd2 != d; ++dd2)
					gsl_matrix_set(data->RR, dd1, dd2, projection[ii * dy * dy + dd1 * dy + dd2]);
		else data->RR = NULL;
		++data;
	}
	data -= N;
