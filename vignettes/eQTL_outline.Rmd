...
We will call the data from these tests `strong` and `random`.
In our eQTL application in Urbut et al,
the `strong` data contained about 16k tests (the top
eQTL per gene), and for the `random` data we used 200k randomly-selected tests.

## Analysis strategy outline

The basic analysis strategy is now:

1. Learn data-driven covariance matrices using `strong` tests.

2. Fit the mashr model to the `random` tests, to learn the mixture weights on all the different covariance matrices and scaling coefficients.

3. Compute posterior summaries on the `strong` tests, using the model fit from step 2. (At this stage
...
the strong hits inside of mash. But in practice you may of course
want to use methods outside of R to extract the matries of data corresponding
to strong and random tests.
```{r}
library(ashr)
library(mashr)
set.seed(1)
...
random.subset = sample(1:40000,5000)

data.strong = mash_set_data(simdata$Bhat[strong.subset,],simdata$Shat[strong.subset,])
data.random = mash_set_data(simdata$Bhat[random.subset,],simdata$Shat[random.subset,])
```
