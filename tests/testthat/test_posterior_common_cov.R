...
test_that("posterior calculations with common cov match regular version",{
  ...
  out1 = calc_post_rcpp(t(data$Bhat),t(data$Shat),data$V, matrix(0,0,0), simplify2array(Ulist),t(posterior_weights), TRUE, FALSE)
  out2 = calc_post_rcpp(t(data$Bhat),t(data$Shat),data$V, matrix(0,0,0), simplify2array(Ulist),t(posterior_weights), FALSE, FALSE)
  expect_equal(out1,out2)
}
)
